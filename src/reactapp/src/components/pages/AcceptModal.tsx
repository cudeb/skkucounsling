...
const AcceptModal: React.FC<AcceptModalProps> = ({
  ...
  const onClose = () => {
    ...
    onCloseModal(cur_year+"-"+cur_month+"-"+cur_date, 
    day_eng[new Date(cur_year,cur_month-1,cur_date).getDay()]+(Number(cur_time)-9).toString());
  };
  ...
  useEffect(() => {
      setCurYear(Number(counselorApprovalStore.auto_recommend.date.split("-")[0]));
      setCurMonth(Number(counselorApprovalStore.auto_recommend.date.split("-")[1]));
      setCurDate(Number(counselorApprovalStore.auto_recommend.date.split("-")[2]));
      setCurTime((Number(counselorApprovalStore.auto_recommend.time)+9).toString());
        ...
        <ModalBody>
          <VStack alignItems={"flex-start"} padding={"1rem"}>
            ...
            <Text fontSize="smaller" fontWeight="600">
              {" "}
              첫 상담일: {cur_year}년 {cur_month}월 {cur_date}일 ({day[new Date(cur_year,cur_month-1,cur_date).getDay()]}) {cur_time}시
            </Text>
            <HStack width="100%" alignItems={"flex-start"} justifyContent={"space-between"} mt={7}>
              <VStack mr={10}>
                <Text fontSize="xl" fontWeight="600" mb={3}>
                  {cur_month}/{cur_date} ({day[new Date(cur_year,cur_month-1,cur_date).getDay()]})
                </Text>
                <RadioGroup
                  defaultValue="10"
