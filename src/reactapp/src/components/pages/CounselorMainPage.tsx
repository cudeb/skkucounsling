...
const CounselorMainPage = () => {
  ...
  const renderFilteredStudents = (typeNum: number): Array<string> => {
    ...
    detailInfo.filter((info) => info.counseling_type === `personal_${typeNum}` && basicInfo.find((basic) => basic.counseling_application.id === info.id))
      .forEach((e) => {
            ...
            >
              <VStack style={{ alignItems: "flex-start" }}>
                ...
                {renderFilteredStudents(1).map((line, index) => (
                  <Text key={index} fontSize="sm">• {line} 학생</Text>
                ))}
              </VStack>
              <VStack style={{ alignItems: "flex-start" }}>
              ...
              <Text fontSize="xl" fontWeight="bold" color="#00953D">다음 상담일: 11월 25일 (수) 11:00</Text>
            </VStack>
          </VStack>
          <VStack
            style={{
