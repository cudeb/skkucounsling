...
const StudentApplyPage = observer(() => {
  ...
  const closeConfirmModal = () => {
    studentApplyStore.isApplySuccess = false;
  ...
  const ClickApply = () => {
    ...
    if (!concent)
      alert("개인정보 수집·이용과 민감정보 처리에 동의가 필요합니다.");
    else if (!["personal_1", "personal_5", "personal_10"].includes(type))
      alert("상담 종류를 선택해주세요.");
    else if (fields.length < 1) alert("상담 분야를 하나 이상 선택해주세요.");
    else if (times.length < 1)
      alert("희망 상담 시간을 하나 이상 선택해주세요.");
    else if (!["10", "11", "12", "13", "14", "15", "16"].includes(testTime))
      alert("희망 심리 검사 시간을 선택해주세요.");
  ...
  // 상담 예정일 구하기
  function isExpectedDay(date: Date) {
    // expected_date의 요일이 times에 있는지 확인하는 함수
    const dayIndex = date.getDay();
    const dayAbbreviation = ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"][
      dayIndex
    ];
    return times.some(
      (time) => typeof time === "string" && time.includes(dayAbbreviation)
    );
  }
    ...
    return (
      ...
      >
        ...
        <Text style={{ textAlign: "start" }} fontSize="md">
          카운슬링센터에서는 학생상담 서비스를 제공하는데 필요한 개인정보를
          수집·이용함에 있어 아래의 내용으로 동의를 받고 있습니다.
        </Text>

        <Text
          style={{ textAlign: "start", marginLeft: "1rem", marginTop: "1rem" }}
          fontSize="md"
        >
          개인정보 수집･이용 내역
          <br />
        ...
        <UnorderedList
          style={{ textAlign: "start", marginLeft: "2rem" }}
          fontSize="md"
        >
          <ListItem>수집·이용 목적 : 학생 심리상담 지원</ListItem>
          <ListItem>
            수집 항목 : 이름, 나이(생년월일), 성별, 소속, 학번, 연락처, 주소,
            가족사항 등 상담신청서 기재사항
          </ListItem>
          <ListItem>
            보유기간 : 5년 (※ 향후 다시 카운슬링센터에 방문할 경우, 효과적인
            상담진행 및 학생의 편의를 위함이며, 보유기간 이후 개인정보는 모두
            파기)
          </ListItem>
          <ListItem>
            귀하는 위 개인정보 수집·이용 동의에 거부할 권리가 있습니다. 단,
  ...
  const CounselingType = observer(() => {
    return (
      ...
      >
        ...
        >
          <Stack>
            <Radio colorScheme="green" value="personal_1">
              검사 해석 상담(검사 후 1회기 해석)
            </Radio>
            <Radio colorScheme="green" value="personal_5">
              5회기 개인 상담
            </Radio>
            <Radio colorScheme="green" value="personal_10">
              10회기 개인 상담
            </Radio>
          </Stack>
        </RadioGroup>
  ...
  const FormUploader = observer(() => {
    return (
      ...
      >
        ...
        <Text style={{ textAlign: "start" }} fontSize="md">
          신청서 양식 다운로드 후 신청서를 작성하여 업로드해주세요.
          <br />* 한컴오피스 한글(.hwp) 및 MW오피스 워드용(.docx) 선택하여
          작성해주세요.
        </Text>
  ...
  const SelectSchedule = observer(() => {
    return (
      ...
      >
        ...
        <Text style={{ textAlign: "start" }} fontSize="md">
          상담에 앞서 심리 검사를 진행합니다. 검사 소요 시간은 개인마다 상이하며
          한 시간 이상 여유를 두는 것을 추천합니다.
        </Text>
        ...
        >
          <VStack>
            <Text fontSize="lg">
              {testYear}/{testMonth}/{testDate}(
              {day[new Date(testYear, testMonth - 1, testDate).getDay()]})
            </Text>
            <RadioGroup
              defaultValue="10"
              style={{ marginTop: "1rem" }}
      ...
      >
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              신청일: {appliedAt?.getFullYear()}년 {appliedAt?.getMonth() + 1}월{" "}
              {appliedAt?.getDate()}일 ({day[appliedAt.getDay()]})<br />
              상담 종류: {getFormattedCounselingType(type)}
              <br />
              심리 검사일: {testYear}년 {testMonth}월 {testDate}일 (
              {day[new Date(testYear, testMonth - 1, testDate).getDay()]}){" "}
              {testTime}시 <br />
              상담 예정일: {expectedDate?.getFullYear()}년{" "}
              {expectedDate?.getMonth() + 1}월 {expectedDate?.getDate()}일 (
              {day[expectedDate.getDay()]})<br />
            </AlertDialogBody>
            <AlertDialogFooter>
              <Button
                ref={cancelRef}
