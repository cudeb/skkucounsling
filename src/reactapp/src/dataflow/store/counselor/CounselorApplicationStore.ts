...
class CounselorApplicationStore {
        ...
        remote
            ...
            .onSuccess((json: any) => {
                const couselingApplications: ICounselingApplicationDetail[] = json.counseling_applications.
                    filter((application: ICounselingApplicationDetail) => !application.denied && !application.approved);
    ...
    fetchFilteredCouselingApplications = (counselType: string, counselField: string, desiredDay: string, desiredTime: string) => {
        remote
            .get("counseling/applications/")
            .onSuccess((json: any) => {
                const couselingApplications: ICounselingApplicationDetail[] = json.couseling_applications.
                    filter((application: ICounselingApplicationDetail) => !application.denied && !application.approved);

                const filteredByType = couselingApplications.filter((application: ICounselingApplicationDetail) => application.counseling_type === counselType);

                const filteredByField = filteredByType.filter((application: ICounselingApplicationDetail) =>
                    application.counseling_preferfields.some(f => f.field === counselField));

                const desiredDayAndTime: string = desiredDay + desiredTime;
                const filteredByTimeSlot = filteredByField.filter((application: ICounselingApplicationDetail) =>
                    application.counseling_prefertimeslots.some(f => f.timeslot === desiredDayAndTime));
    ...
    fetchhCurrentApplication = (applicationId: number) => {
        remote
            .get("counseling/applications/")
            .onSuccess((json: any) => {
                const currentApplication: ICounselingApplicationDetail[] = json.counseling_applications.
                    filter((application: ICounselingApplicationDetail) => application.id == applicationId);
